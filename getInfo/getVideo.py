...
def get_media_info(file):
    # pname = 'D:\Program Files\MediaInfo\MediaInfo.exe "%s" --Output=JSON' % (file)
...
def get_dict_data(json_data):
    # 获取文件大小
    filesize = json_data['tracks'][0]['other_file_size'][4]
    # 获取码率
    malv = json_data['tracks'][0]['overall_bit_rate']
    # 获取播放时长
    duration = json_data['tracks'][0]['other_duration'][4]
    # 获取文件类型
    file_format = json_data['tracks'][0]['format']
    # 获取帧宽
    samp_width = json_data['tracks'][1]['width']
    # 获取帧高
    samp_height = json_data['tracks'][1]['height']
    # 获取帧率
    frame_rate = json_data['tracks'][0]['other_frame_rate'][0]
    # 获取备注 comment
    info_dict = json_data['tracks'][0]
    # print(dir(info_dict))
    # print(info_dict.get('comment', ''))
    # print(json_data['tracks'][0])
    comment = info_dict.get('comment', '')
    # 获取标题
    title = info_dict.get('title', '')
    return [filesize, malv, duration, file_format, samp_width, samp_height, frame_rate, comment, title]
...
sh.write(row_count, 7, "帧率")
sh.write(row_count, 8, "备注")
sh.write(row_count, 9, "标题")
...
for item in dict_all:
    ...
    sh.write(row_count, 7, dict_all[item][6])
    sh.write(row_count, 8, dict_all[item][7])
    sh.write(row_count, 9, dict_all[item][8])
    row_count += 1
#
wb.save("元数据统计.xls")
#
